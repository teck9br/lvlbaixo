...
// .select().single() / .update().eq()). Each table gets one canned
// {data, error} result, reused across every chained call for that table —
// enough for the simple, single-purpose queries in our route handlers.
